...
class HotelProvider : ContentProvider() {
    ...
    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<String>?): Int {
        ...
        val rowsDeleted = when (uriType) {
            TYPE_HOTEL_DIR -> sqlDB.delete(TABLE_HOTEL, selection, selectionArgs)
            TYPE_HOTEL_ITEM -> {
                val id = uri.lastPathSegment
                sqlDB.delete(
    ...
    override fun query(
        ...
        val cursor: Cursor
        when (uriType) {
            TYPE_HOTEL_DIR -> cursor = queryBuilder.query(
                sqlDB, projection, selection,
                selectionArgs, null, null, sortOrder
            )
            TYPE_HOTEL_ITEM -> {
                queryBuilder.appendWhere("$COLUMN_ID = ?")
                cursor = queryBuilder.query(
                    sqlDB, projection, selection, arrayOf(uri.lastPathSegment),
                    null, null, null
                )
            }
            else -> throw IllegalArgumentException("Uri não suportada: $uri")
