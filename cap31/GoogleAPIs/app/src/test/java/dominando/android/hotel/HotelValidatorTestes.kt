...
class HotelValidatorTestes {
    ...
    @Test
    fun should_validate_info_for_a_valid_hotel() {
        //JUnit4 style
    ...
    @Test
    fun should_not_validate_info_without_a_hotel_name() {
        val missingName = validInfo.copy(name = "")
        assertThat(validator.validate(missingName)).isFalse()
    }

    @Test
    fun should_not_validate_info_without_a_hotel_address() {
        val missingAddress = validInfo.copy(address = "")
        assertThat(validator.validate(missingAddress)).isFalse()
    }

    @Test
    fun should_not_validate_info_when_name_is_outside_accepted_size() {
        val nameTooShort = validInfo.copy(name = "I")
        assertThat(validator.validate(nameTooShort)).isFalse()
    }

    @Test
    fun should_not_validate_info_when_address_is_outside_accepted_size() {
        val bigAddress =
            "Av. Brigadeiro Eduardo Gomes de Brito, 546, Lagoa da Ante, Maceió - AL, 57038-230"
